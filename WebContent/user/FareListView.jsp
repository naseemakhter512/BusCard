<%@page import="in.co.buscard.common.model.FareModel"%>
...
<div class="inner-block" style="padding: 5em 1em 4em 1em;">
...
<div class="chit-chat-layer1" style="position: relative;flex-direction: column;display: flex;min-width: 0;word-wrap: break-word;background-color: #fff;background-clip: border-box;border: 1px solid rgba(0, 0, 0, 0.125);border-radius: 0.25rem;">
	<div style="padding: 0.75rem 1.25rem; margin-bottom: 0;background-color: rgba(0, 0, 0, 0.03);border-bottom: 1px solid rgba(0, 0, 0, 0.125);">
					<div class="row">
						<div class="col-md-7">
							<i class="fa fa-table"></i> Fare
						</div>
						<div class="col-md-3">							
							<input type="text"id="demo" placeholder="Search"  class="form-control emailfind">
						</div>
						<div class="col-md-2">
							<a href="/BusCard/FareCtl" class="btn btn-info btn-block" title="User">Add New</a>
						</div>
					</div>	
				</div>
	...
	<div class="table-responsive" style="overflow-x:auto;">		
	  <form action="<%=ORSView.FARE_LIST_CTL%>">				
							...
							<thead>
								<tr>
                                      ...
                                      <th>MIN_KM</th>
                                       <th>MAX_KM</th>
                                       <th>GENERAL_FARE</th>
                                       <th>SENIOR_CITIZEN_FARE</th>
                                       <th>FEMALE_FARE</th>
                                       <th>JUNIOR_STUDENT</th>
                                        <th>SENIOR_STUDENT</th>
		...
		%>
			<tr>
				<td colspan="3"><%=HTMLUtility.getErrorMessage(request)%></td>
			</tr>
			<%
			}
		%>
			...
			Iterator<FareModel> it = list.iterator();
			while (it.hasNext()) {
				FareModel model = it.next();
		%>
                                <tr>
                                  <td><span><%=model.getId()%></td>
                                  <td><%=model.getMinKm()%></td>
                                 <td><%=model.getMaxKm()%></td>
                                 <td><%=model.getGeneralFare()%></td>
                                 <td><%=model.getSeniorCitizenFare()%></td>
                                 <td><%=model.getFemaleFare()%></td>
                                  <td><%=model.getJuStudentFare()%></td>
                                   <td><%=model.getSrStudentFare()%></td>
                                                  	<td>
														<a href="<%=ORSView.FARE_CTL%>?id=<%=model.getId()%>">
															<i class="fa fa-edit text-info"></i>
														</a>
														&nbsp;														
													<a href="<%=ORSView.FARE_LIST_CTL%>?operation=Delete&id=<%=model.getId()%>">	<i class="fa fa-trash text-danger" style="cursor:pointer"></i>
													</a>
													</td>
                              </tr>
                            	<%
